$(document).ready(function () {
    ...
    function initializeDataTable() {
        ...
        info: "Showing _TOTAL_ Rows",
        emptyTable: "No data available",
        infoEmpty: "Showing 0 to 0 of 0 entries",
        infoFiltered: "(Records filtered from total _MAX_ data)"
                    ...
                    api.columns().eq(0).each(function (colIdx) {
                        if (colIdx === api.columns().count() - 1) return; // Lewati kolom terakhir (Aksi)
                        var $input = $('input', api.column(colIdx).header());
                        if ($input.length) { // Pengecekan apakah $input ada
                            // Hapus event sebelumnya untuk mencegah duplikat
                            $input.off('keydown keyup change clear click focus');
                            $input.on('keydown', function (e) {
                                // Tangani Ctrl+A secara eksplisit
                ...
                $input.on('keyup change clear', function (e) {
                    // Lakukan pencarian hanya jika bukan Ctrl+A
                    if (!(e.ctrlKey && e.keyCode === 65)) {
                ...
                console.warn(`No input found in column header for column index ${colIdx}`);
            }
        });
    }
                });
